...
ALTER TABLE `campaign` DROP PRIMARY KEY,
    ...
    MODIFY `status` ENUM('DRAFT', 'READY', 'SCHEDULED', 'RUNNING', 'SENDING', 'PAUSED', 'COMPLETED', 'CANCELLED', 'FAILED') NOT NULL DEFAULT 'DRAFT',
    ADD PRIMARY KEY (`id`);

-- AlterTable
ALTER TABLE `campaignrecipient` MODIFY `campaign_id` VARCHAR(191) NOT NULL;
