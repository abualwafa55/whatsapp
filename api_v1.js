...
const baileysModuleReady = import('@whiskeysockets/baileys')
    .then(module => {
        ({ jidNormalizedUser } = module);
    })
    .catch(error => {
        console.error('Failed to load @whiskeysockets/baileys. Ensure a compatible Node.js version is in use.', error);
        process.exit(1);
    });
...
+9988776655,Charlie Davis,Product Manager,Innovation Labs`;
    ...
    async function sendMessage(sock, to, message) {
        try {
            const jid = await normalizeJid(to);
